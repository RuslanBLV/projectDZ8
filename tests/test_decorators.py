...
@pytest.mark.parametrize(
    "card_numbers, expected",
    [
        ("35383033474447895560", "get_mask_account ok: **5560\n"),
...
def test_log_filter_by_state():
    ...
    assert filter_by_state(list_state) == (
        "filter_by_state ok: [{'id': 41428829, 'state': 'EXECUTED', 'date': "
...
@pytest.mark.parametrize(
    "data, expected",
    [
        ("2024-03-11T02:26:18.671407", "get_date ok: 11.03.2024\n"),
...
def test_log_generator_filter_by_currency(capsys):
    ...
    assert (next(generator_usd)) == (
        "filter_by_currency ok: {'id': 939719570, 'state': 'EXECUTED',"
    ...
    assert (next(generator_usd)) == (
        "filter_by_currency ok: {'id': 142264268, 'state': 'EXECUTED',"
    ...
    assert (next(generator_usd)) == (
        "filter_by_currency ok: {'id': 895315941, 'state': 'EXECUTED',"
    ...
    assert (next(generator_rub)) == (
        "filter_by_currency ok: {'id': 873106923, 'state': 'EXECUTED',"
    ...
    assert (next(generator_rub)) == (
        "filter_by_currency ok: {'id': 594226727, 'state': 'CANCELED',"
